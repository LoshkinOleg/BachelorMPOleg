...
if len(sys.argv) != 2:
    print("Please pass the relative path to the SOFA file as argument to the command line.")
    quit()

# Open the file for reading
file = SOFAFile(sys.argv[1],"r")

# Check validity
if file.isValid():
    print(sys.argv[1] + " is a valid SOFA file")
else:
    print(sys.argv[1] + " is _NOT_ a valid SOFA file")
    # exit()
...
if convention == 'AmbisonicsDRIR':
    conventionFile = SOFAAmbisonicsDRIR(sys.argv[1],"r")
elif convention == 'GeneralFIR':
    conventionFile = SOFAGeneralFIR(sys.argv[1],"r")
elif convention == 'GeneralFIRE':
    conventionFile = SOFAGeneralFIRE(sys.argv[1],"r")
elif convention == 'GeneralTF':
    conventionFile = SOFAGeneralTF(sys.argv[1],"r")
elif convention == 'MultiSpeakerBRIR':
    conventionFile = SOFAMultiSpeakerBRIR(sys.argv[1],"r")
elif convention == 'SimpleFreeFieldHRIR':
    conventionFile = SOFASimpleFreeFieldHRIR(sys.argv[1],"r")
elif convention == 'SimpleFreeFieldSOS':
    conventionFile = SOFASimpleFreeFieldSOS(sys.argv[1],"r")
elif convention == 'SimpleHeadphoneIR':
    conventionFile = SOFASimpleHeadphoneIR(sys.argv[1],"r")
elif convention == 'SingleRoomDRIR':
    conventionFile = SOFASingleRoomDRIR(sys.argv[1],"r")
else:
    print(convention + " is _NOT_ a valid SOFA convention type")
    # exit()

if conventionFile.isValid():
    print(sys.argv[1] + " is a valid " + convention + " SOFA file")
else:
    print(sys.argv[1] + " is _NOT_ a valid " + convention + " SOFA file")
    # exit()
